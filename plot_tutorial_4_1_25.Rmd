...
This tutorial will walk you through some code to create a rather complicated map using ggplot 2. We'll use multiple layer types and go deep into the capabilities of ggplot2 to create this publication-ready map, and you'll learn lots of tricks that you can use in your own maps along the way!

First we'll read in our camera trap point data.
```{r}
#### load in camera locations
points <- st_read(here("data", "plot_data", "points_wa.shp"))
points <- st_as_sf(points)
```


Next we'll load in some polygon data: our state, county, and census tract shapefiles.
...
envdat <- st_read(here("data", "env_health_data", "env_health_all_KP.shp")) %>%
  st_transform(st_crs(water))


#now lets make one big dataframe so we can create percentiles for our map 
colnames(envdat)
...
ggsave(filename = "map_high_res.bmp", path = here("plots"), width = 25, height = 25, device='bmp', dpi=300)
